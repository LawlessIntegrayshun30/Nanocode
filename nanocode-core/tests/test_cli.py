...
def test_cli_rejects_store_from_different_program(tmp_path: Path):
    program_src = """
    (program demo
      (root (seed :scale 0))
      (rules
    ...
    program_file.write_text(program_src.replace("seed", "other"))
    ...
    assert "digest mismatch" in resumed.stderr
