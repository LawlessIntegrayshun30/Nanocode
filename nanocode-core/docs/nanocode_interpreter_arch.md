...
- The CLI summary includes per-rule and per-scale counters (`rule_counts`/`scale_counts`) alongside the frontier, store size, idle/budget exhaustion flags to highlight which rules fired, how much work remains, and whether a step budget halted execution.
- `--store-json` writes the term store snapshot to disk with root/frontier/processed metadata so runs can be replayed or inspected offline without re-running the program. Pair with `--steps-only` to capture mid-flight snapshots.
- `--load-store` bootstraps the runtime from a stored snapshot, honoring the persisted scheduler choice by default.
