...
- `--load-store` bootstraps the runtime from a stored snapshot, honoring the persisted scheduler/strict/walk settings by default (override with `--no-walk-children` or `--strict-matching/--no-strict-matching`).
