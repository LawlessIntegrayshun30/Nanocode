...
Snapshot bundles carry a deterministic `program_digest` derived from the parsed program. The CLI refuses to resume when the
loaded store and the current program disagree, preventing accidental reuse of stale or mismatched snapshots during iteration.
...
- `snapshot_population` / `load_population` checkpoint and rehydrate genome populations with embedded fingerprints and annotations, optionally validating structural constraints on reload so evolutionary searches can pause/resume without losing determinism or provenance.
