...
from src.fingerprint import fingerprint_genome
from src.terms import Term
from src.terms import term_from_dict, term_to_dict
...
class Genome:
    ...
    def fingerprint(self) -> str:
        return fingerprint_genome(self)
...
def _ensure_constraints(root: Term, constraints: StructuralConstraints | None) -> None:
    if constraints is None:
        return
    violations = validate_structure(root, constraints)
    if violations:
...
def genome_to_dict(genome: Genome) -> dict:
    """Serialize a genome with its fingerprint for deterministic snapshots."""

    payload = {
        "root": term_to_dict(genome.root),
...
def genome_from_dict(data: Mapping[str, object], *, constraints: StructuralConstraints | None = None) -> Genome:
    """Rehydrate a genome from a JSON-friendly mapping, validating consistency."""

    root_payload = data["root"]
    root = term_from_dict(root_payload)  # type: ignore[arg-type]
    _ensure_constraints(root, constraints)

    annotations = data.get("annotations")
    genome = Genome(root=root, annotations=annotations if isinstance(annotations, dict) else None)
    expected_fp = data.get("fingerprint")
    if expected_fp is not None and str(expected_fp) != genome.fingerprint:
        raise ValueError(
            f"fingerprint mismatch for genome: expected {expected_fp}, got {genome.fingerprint}"
        )
    return genome


def snapshot_population(
    population: Sequence[Genome], *, constraints: StructuralConstraints | None = None
) -> dict:
    """Snapshot a population into a JSON-friendly mapping with fingerprints."""

    genomes = []
    for genome in population:
        _ensure_constraints(genome.root, constraints)
        genomes.append(genome_to_dict(genome))
    return {"genomes": genomes}


def load_population(payload: Mapping[str, object], *, constraints: StructuralConstraints | None = None) -> list[Genome]:
    """Load a population from a snapshot created by :func:`snapshot_population`."""

    genomes_data = payload.get("genomes")
    if genomes_data is None:
        raise ValueError("snapshot missing 'genomes' field")

    genomes: list[Genome] = []
    for entry in genomes_data:  # type: ignore[assignment]
        genomes.append(genome_from_dict(entry, constraints=constraints))
    return genomes
