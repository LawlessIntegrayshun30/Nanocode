...
from src.fingerprint import fingerprint_program
...
def run_cli(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        ...
        program_digest = fingerprint_program(program)
        ...
        state_digest = state.get("program_digest") if state else None
        if state_digest is not None and state_digest != program_digest:
            raise ValueError(
                "Loaded store was produced from a different program (digest mismatch)"
            )
        ...
        if state:
            ...
            runtime.load_state(
                ...
                constraints=constraints,
                program_digest=program_digest,
                state_program_digest=state_digest,
            )
        else:
